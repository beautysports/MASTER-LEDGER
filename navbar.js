...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (authError || !userData || !userData.user) {
        // If not logged in, boot them back to index.html
    ...
    const role = profile ? profile.role : 'staff'; // Default to lowest privilege if error
    ...
    style.innerHTML = `
        ...
        @media (max-width: 768px) {
            .sys-nav-links { display: none; }
            /* You can expand on a mobile hamburger menu later if needed, but flex-wrap handles basics */
    ...
    let linksHtml = `
        <a href="dashboard.html" class="sys-nav-link ${currentPage === 'dashboard.html' ? 'active' : ''}">Dashboard</a>
    `;
